...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['registrar_entrada'])) {
    ...
    $orden_compra = $_POST['numero_orden_compra'];
    $fecha = $_POST['fecha_entrada'];
    $usuario_id = $_SESSION['usuario_id'];
    $doc_referencia = "Fact: $factura / OC: $orden_compra";
    ...
    try {
        ...
        $sql_insert = "INSERT INTO movimientos_inventario 
                        (producto_id, usuario_id, tipo, cantidad, doc_referencia, fecha_movimiento) 
                       VALUES 
                        (:pid, :uid, 'entrada', :cant, :doc, :fecha)";
        $stmt_insert = $conn->prepare($sql_insert);
        $stmt_insert->execute([
            ':pid' => $producto_id, ':uid' => $usuario_id, ':cant' => $cantidad,
            ':doc' => $doc_referencia, ':fecha' => $fecha
        ]);
        ...
        $conn->rollBack();
        $mensaje = "⚠️ Error al registrar la entrada: " . $e->getMessage();
    }
}
...
<head>
    <meta charset="UTF-8">
    <title>Entrada de Productos - SWIBO</title>
    ...
    <main class="main-content">
        ...
        <div class="form-container">
            ...
            <form action="entrada_producto.php" method="POST">
                ...
                <div class="form-grid">
                    <div class="input-group"><label>Cantidad a Ingresar</label><input type="number" name="cantidad" min="1" required></div>
                    <div class="input-group"><label>Fecha de Entrada</label><input type="date" name="fecha_entrada" value="<?php echo date('Y-m-d'); ?>" required></div>
                    <div class="input-group"><label>Nº de Factura</label><input type="text" name="numero_factura"></div>
                    <div class="input-group"><label>Nº de Orden de Compra</label><input type="text" name="numero_orden_compra"></div>
                </div>
                <button type="submit" name="registrar_entrada" class="btn-login" style="margin-top: 20px;">Registrar Entrada</button>
